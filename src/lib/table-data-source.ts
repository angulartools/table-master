...
export class TableDataSource<T> extends DataSource<TableElement<T>> {
  ...
  constructor(
    data: T[],
    dataType?: new () => T,
    private validatorService?: TableDataValidator,
    private config = { prependNewElements: true, suppressErrors: false }) {
    super();
  ...
  protected checkValidatorFields(validatorService: TableDataValidator) {
